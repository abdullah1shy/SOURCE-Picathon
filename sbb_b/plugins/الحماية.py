...
#  فريق بيكاثون + ترجمه وكتابة فريق جمثون
...
حاليا متكدر بعد تحجي الى ان يجي صاحب الحساب ويشيل  الحظر. 🙂💘"
...
**اتمنى ما تكرر الرسائل حتى ما اضطر احظرك 😕🌿**"""
    ...
    USER_BOT_WARN_ZERO = f"**اتذكر حذرتك وقلت لك لا تكرر اكثر من خيار ولا ترسل رسائل مجرد انتظر. \
تعتقد راح اخليك تكرر لا اسف راح احظرك من الحساب. \
حاليا متكدر بعد تحجي الى ان ياتي صاحب الحساب ويقوم بالغاء الحظر. 🙂💘**"
    await event.reply(USER_BOT_WARN_ZERO)
    await event.client(functions.contacts.BlockRequest(chat.id))
    the_message = f"#حماية الخاص\
                \n[{get_display_name(chat)}](tg://user?id={chat.id}) is blocked\
...
- اختر  خيار واحد فقط لنعرف سبب قدومك هنا 🤍
...
- هذه الخيارات في الاسفل اختر واحد فقط ⬇️"""
    buttons = [
        ...
        (Button.inline(text="للدردشة مع مالك الحساب.", data="to_chat_with_my_master"),),
        (
            Button.inline(
...
**لكن في الوقت الحالي لا تكرر ارسال الرسائل حتى لا اضطر لحظرك 🙁💞**"""
